...
```{block2, type='boxinfo'}
**Using the Pustejovsky-Rogers Approach Directly in `metabias`**

In the latest versions of **{meta}**, the `metabias` function also contains an option to conduct Eggers' test with the corrected standard error formula proposed by Pustejovsky and Rogers. The option can be used by setting `method.bias` to `"Pustejovsky"`.
