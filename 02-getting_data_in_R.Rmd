...
<<<<<<< HEAD
As per usual, such data is stored in **EXCEL spreadsheets**. We recommend to store your data there, because this makes it very easy to import data into RStudio. However, it is very important how you **name the columns of your spreadsheet**. If you name the columns of your sheet adequately in EXCEL already, you can save a lot of time because your data doesn't have to be transformed in RStudio later on. 
=======
...
<<<<<<< HEAD
madata.s$`intervention duration`=NULL
kable(madata.s) %>%
  column_spec(6)
```
...
<<<<<<< HEAD
It may often come in handy to **select certain studies for further analyses**, or to **exclude some studies in further analyses** (e.g., if they are outliers). To do this, we can use the `filter` function in the `dplyr`package, which is part of the `tidyverse` package we installed before. So, let's load the package first.
=======
