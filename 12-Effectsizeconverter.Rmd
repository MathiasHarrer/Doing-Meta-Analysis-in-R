...
A common format to calculate the SMD in single trials is **Cohen's d** [@cohen1988statistical]. Yet, this summary measure has been **shown to have a slight bias in small studies, for which it overestimates the effect** [@hedges1981distribution].
...
To calculate the effect sizes, we will use Daniel Lüdecke's extremely helpful `esc` package [@esc]. So, please **install this package first** using the `install.packages("esc")` command, and then load it in your library.
...
3.  [Calculating an Odds-Ratio from *chi-square*](#c)
...
## Calculating an Odds-Ratio from *Chi-square* {#c}
