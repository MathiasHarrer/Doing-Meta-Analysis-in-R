...
To perform multimodel inference, we have prepared a function for you, `mreg.multimodel.inference`. The full code for this function can be found [here](https://github.com/MathiasHarrer/Doing-Meta-Analysis-in-R/blob/master/mreg.multimodel.inference.R). Again, R doesn't know this function yet, so we have to let R learn it by **copying and pasting** the code from the website **in its entirety** into the **console** on the bottom left pane of RStudio, and then hit **Enter ⏎**. For the `mreg.multimodel.inference` function, the following parameters need to be specified:

```{r,echo=FALSE}
i<-c("TE", "seTE", "data", "predictors", "method", "test", "level")
ii<-c("The name of the variable containing the calculated effect sizes for each study.", "The name of the variable containing the calculated standard error of each effect size.", "Our dataset containing all effect size and predictor data.", "The predictors in our dataset we want to consider. We have to supply them to the function as predictors=c('variable1', 'variable2', ...).","The tau-squared estimator we want to use. We can choose from 'Maximum-Likelihood ('ML') and Restricted Maximum Likelihood ('REML'; default).", "The test we want to use to evaluate the significance of a predictor.'knha' (Knapp-Hartung) is used as the default, but we can also set this to 'z'.", "This specifies if we want to also model all possible interaction terms (level=2) or not (level=1). Default is level=1.")
ms<-data.frame(i,ii)
names<-c("Parameter", "Function")
colnames(ms)<-names
kable(ms) 
...
The `mreg.multimodel.inference` requires the `glmulti` package to be installed on you computer, so let's make sure first to install this package and then load it into our library.
...
`glmulti` requires the installation of `rJava` as a dependency. Usually, this is done automatically when installing `glmulti`. However, `rJava` installation often **does not work properly**, and `rJava` may cease to function after updates are installed. If this happens on your computer, this [thread](https://stackoverflow.com/questions/6492361/problem-loading-rjava), or a quick Google search may help you to fix this problem.
```
...
source("mreg.multimodel.inference.R")
load("mvreg_data.rda")
library(glmulti)
set.seed(123)
```
...
```{r, warning=FALSE, message=FALSE}
mreg.multimodel.inference(TE = "yi", 
                          seTE = "sei",
                          data = mvreg.data,
...
```{block,type='rmdinfo'}
The output of the `mreg.multimodel.inference` function displayed after running the function is **only a small part of all results returned by the function**. If we want to access all results of the function, we can save the results to an object (e.g. `results <- mreg.multimodel.inference(...)`), and then open them using the 'dollar' operator. The following results are returned, among other things:
...
* `model.details`: a list containing the detailed `rma` output for all fitted models. We can access a specific model by its number the following way: `results$model.details[[1]]` (for the first model).
```
