...
![](C:/Users/Admin/Documents/R/WORKING_DIRECTORY/Meta-Analyse Buch/bookdown-demo-master/chappool.jpg)
...
### Setting the columns of excel spreadsheet {#excel_preparation}
...
* The **names** of the individual studies, so that the can be easily identified later on. Usually, the first author of a study is used for this (e.g. "Ebert et al., 2018")
...
* If you're using a PC or Mac, it is advised to save your EXCEL sheet as a **comma-separated-values (.csv) file**. This can be done by clicking on "save as" and then choosing (.csv) as the output format.
...
library(png)
library(grid)
img <- readPNG("C:/Users/Admin/Documents/R/WORKING_DIRECTORY/Meta-Analyse Buch/bookdown-demo-master/snippet.PNG")
grid.raster(img)
```
...
2. An import assistant should now pop up, which is also loading a preview for data. This can be time consuming sometimes, so you can skip this step if you want to, and klick straight on **"import"**

```{r, echo=FALSE, fig.width=7,fig.height=3}
library(png)
library(grid)
img <- readPNG("C:/Users/Admin/Documents/R/WORKING_DIRECTORY/Meta-Analyse Buch/bookdown-demo-master/snippet_3.PNG")
grid.raster(img)
```
...
We can see now that `ROB`is indeed a **character** type variable, which contains only to words: "low" and "high". We want to convert this to a **factor** variable now, which has only two levels, low and high. To do this, we use the `as.factor()` function.
...
Now lets have a look at the **intervention type** subgroup variable. This variable is corrently stores as a character too.
...
The R code to store theses three studies in a new dataset called `madata.new` looks like this:
